...
class DeviceDiscoveryService {
  ...
  final void Function(String deviceId, String ip, String bindToken)?
  onBindingSeen;
  ...
  static Future<Map<String, dynamic>> sendCommand(
    ...
    final readOnly =
        cmd == 'get_pair_status' || cmd == 'get_pending_pair_requests';
    if (!readOnly) {
  ...
  /// 演示舵机 40°→120°→40°
  static Future<Map<String, dynamic>> demoServo(
    String host, {
  ...
  /// 单舵机角度
  static Future<Map<String, dynamic>> moveServo(
    String host,
    int servoIndex,
    int angle, {
    int port = Protocol.staTcpPort,
  }) {
    return sendCommand(host, port, {
      'cmd': 'move_servo',
      'servo': servoIndex,
      'angle': angle,
    });
  ...
  /// 获取待处理的配对请求（来自其他 ESP）
  static Future<Map<String, dynamic>> getPendingPairRequests(
    String host, {
    int port = Protocol.staTcpPort,
  }) {
  ...
  static Future<Map<String, dynamic>> acceptPair(
    String host,
    String fromDeviceId, {
    int port = Protocol.staTcpPort,
  }) {
  ...
  /// 拒绝配对（从 pending 列表移除）
  static Future<Map<String, dynamic>> rejectPair(
    String host,
    String fromDeviceId, {
    int port = Protocol.staTcpPort,
  }) {
  ...
  static Future<Map<String, dynamic>> unpair(
    String host, {
    String? peerIp,
    int port = Protocol.staTcpPort,
  }) {
